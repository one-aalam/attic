...
type EntityConstructor = typeof User // | typeof [Entity];
type EntityInstance =  User // | [Entity];
...
export const findOneEntity = async <T extends EntityConstructor>(
    Constructor: T,
    options?: FindOneOptions,
  ): Promise<InstanceType<T>> => {
...
export const findOneEntityOrFail = async <T extends EntityConstructor>(
    Constructor: T,
    options?: FindOneOptions,
  ): Promise<InstanceType<T>> => {
    const instance = await Constructor.findOneOrFail(options);
    return instance as InstanceType<T>
};
